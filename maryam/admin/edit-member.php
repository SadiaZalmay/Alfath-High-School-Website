...
// else{
    if(isset($_POST['submit'])){  
        ...
        $user = $_POST['username'];
        $pass = $_POST['password'];
        $query=mysqli_query($connect,"Update member set fname=' $name',lname='$mam',gender='$gen',email='$ema',contact='$cont',username='$user',password='$pass' where id='$catid'");
        if($query)
        {
...
<?php 
$catid=intval($_GET['cid']);
$query=mysqli_query($connect,"Select id,fname,lname,gender,email,contact,username,password,picture from member");
$cnt=1;
